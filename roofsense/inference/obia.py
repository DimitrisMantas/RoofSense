...
from typing import AnyStr, Final, Protocol, TypedDict
...
class MaskGeneralizer:
    TILE_ID_REGEX: Final[re.Pattern[Protocol[AnyStr]]] = re.compile(
        r"\d{1,2}-\d{3,4}-\d{2,4}"
    )
...
if __name__ == "__main__":
    ...
    for name, lod in zip(
        ...
        gen.generalize(
            src_filepath=r"/dataset/infer_storage/maps/9-284-556.map.tpe.mask.stride16.tif",
            dst_filepath=f"9-284-556.map.mask.stride16.{name}.tif",
            lod=lod,
        )
