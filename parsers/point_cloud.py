...
class PointCloud:
    ...
    def csf(
        ...
        initial_elevation: Optional[float] = None,
    ) -> None:
        """
        ...
        c = np.array(
            [
                *np.meshgrid(x, y),
                z,  # All particles are initially movable.
                np.ones(z.shape),
            ]
        )
        ...
        qrs = (
            np.array([c[0, :, :].reshape(-1), c[1, :, :].reshape(-1)])
            .transpose()
            .tolist()
        )
    ...
    def downsample(
        self,
        percentage: float,
        method: Optional[DownsamplingMethod] = DownsamplingMethod.RANDOM,
    ) -> None:
        """
    ...
    def dtm(
        ...
        filename: str,
        resolution: float,
        method: Optional[DTMGenerationMethod] = DTMGenerationMethod.CLASSIFICATION,
    ) -> None:
        """
