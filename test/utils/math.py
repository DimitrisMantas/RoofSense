...
class TestMath(unittest.TestCase):
    ...
    def test_dbratios_array(self):
        np.testing.assert_array_almost_equal(utils.math.dbratios(np.array([0, 10])),  # NOTE: See previous comment.
                                             np.array([1, 10]))

    def test_decibels_float(self):
        self.assertAlmostEqual(utils.math.decibels(0),  # NOTE: Logarithms are not defined in ℝ_{≤0}.
                               -np.inf)

    def test_decibels_array(self):
        np.testing.assert_array_almost_equal(utils.math.decibels(np.array([0, 1])),  # NOTE: See previous comment.
                                             np.array([-np.inf, 0]))
