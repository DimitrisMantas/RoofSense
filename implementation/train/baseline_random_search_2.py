...
def objective(trial: optuna.Trial):
    ...
    label_smoothing = 0.1
    # We do not adjust this because different optimizers handle it differently.
    # 10 Values
    weight_decay = trial.suggest_float(name="weight_decay", low=0, high=1e-2, step=1e-3)

    # Optimization Algorithm
    optimizer = trial.suggest_categorical(name="optimizer", choices=["adam", "adamw"])

    # Learning Rate Scheduling
    # 5 Values
    warmup_epochs = trial.suggest_int(name="warmup_epochs", low=50, high=150, step=25)
    annealing = trial.suggest_categorical(name="annealing", choices=["cos", "poly"])
...
if __name__ == "__main__":
    study = optuna.create_study(
        storage="sqlite:///optimization_random_search_round_2.db",
